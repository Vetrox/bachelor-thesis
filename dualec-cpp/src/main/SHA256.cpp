...
void SHA256::update(uint8_t const* data, size_t length)
{
...
void SHA256::update(std::string const& data)
{
    update(reinterpret_cast<uint8_t const*>(data.c_str()), data.size());
}
...
std::string SHA256::toString(uint8_t const* digest)
{
    std::stringstream s;
